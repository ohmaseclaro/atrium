# @atriumjs/example-express-host

Minimal **Express** server that mounts **`@atriumjs/express`** only (no bundled React UI). Defaults match the root [`README.md`](../../README.md).
...
For the **full interactive demo** (Vite + React + optional browser chrome + multi-tab), use [`@atriumjs/demo`](../../packages/demo/README.md) instead:
...
pnpm --filter @atriumjs/example-express-host dev
```
...
Use the returned **`wsUrl`** and **`viewerToken`** with any WebSocket client or embed [`@atriumjs/react`](../../packages/react/README.md).

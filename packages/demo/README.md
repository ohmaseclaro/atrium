...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@atriumjs/atrium-server`   | `atrium()` on `/atrium` with in-memory sessions, `authorize` demo principal, **dial** relay to the worker                                        |
| `@atriumjs/atrium-react`    | `<RemoteBrowser />` after `POST /atrium/sessions` (demo uses **`chrome="full"`** + optional status line; see [React README](../react/README.md)) |
| `@atriumjs/atrium-protocol` | Imported in the client bundle for shared types (wire messages match these shapes)                                                                |
| `@atriumjs/atrium-worker`   | Started alongside the UI by `pnpm dev` (waits for `http://127.0.0.1:7070`)                                                                       |
| `@atriumjs/atrium-cli`      | Mentioned in the footer; run `pnpm --filter @atriumjs/atrium-cli exec atrium doctor` from repo root                                                       |
...
```bash
ATRIUM_CHROMIUM_CHANNEL=chrome pnpm --filter @atriumjs/atrium-worker start
```

On Linux without a monitor, run the worker with **Xvfb**, e.g. `xvfb-run -a pnpm --filter @atriumjs/atrium-worker start`, or use the repo **Docker** worker image (uses `xvfb-run` in `CMD`).
...
```bash
pnpm --filter @atriumjs/atrium-demo dev:ui-only
```
...
```bash
pnpm --filter @atriumjs/atrium-demo build
ATRIUM_WORKER_SECRET=… ATRIUM_WORKER_DIAL_BASE=ws://… pnpm --filter @atriumjs/atrium-demo start
```

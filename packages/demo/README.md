...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@atriumjs/express`  | `atrium()` on `/atrium` with in-memory sessions, `authorize` demo principal, **dial** relay to the worker                                        |
| `@atriumjs/react`    | `<RemoteBrowser />` after `POST /atrium/sessions` (demo uses **`chrome="full"`** + optional status line; see [React README](../react/README.md)) |
| `@atriumjs/protocol` | Imported in the client bundle for shared types (wire messages match these shapes)                                                                |
| `@atriumjs/worker`   | Started alongside the UI by `pnpm dev` (waits for `http://127.0.0.1:7070`)                                                                       |
| `@atriumjs/cli`      | Mentioned in the footer; run `pnpm --filter @atriumjs/cli exec atrium doctor` from repo root                                                     |
...
```bash
ATRIUM_CHROMIUM_CHANNEL=chrome pnpm --filter @atriumjs/worker start
```

On Linux without a monitor, run the worker with **Xvfb**, e.g. `xvfb-run -a pnpm --filter @atriumjs/worker start`, or use the repo **Docker** worker image (uses `xvfb-run` in `CMD`).
...
```bash
pnpm --filter @atriumjs/demo dev:ui-only
```
...
```bash
pnpm --filter @atriumjs/demo build
ATRIUM_WORKER_SECRET=… ATRIUM_WORKER_DIAL_BASE=ws://… pnpm --filter @atriumjs/demo start
```

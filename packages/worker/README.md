...
With pnpm: `pnpm add @atriumjs/atrium-worker`, then `pnpm exec atrium-worker`.

From the monorepo (after `pnpm build`), use `pnpm --filter @atriumjs/atrium-worker start`.
...
```bash
pnpm --filter @atriumjs/atrium-worker run build
```
